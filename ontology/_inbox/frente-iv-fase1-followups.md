...
|---|---|---|---|
| 1 | **Usuário de robô no Conexos** | **P1 — AGRAVADO** | O ERP limita sessões simultâneas por usuário (`LOGIN_ERROR_MAX_SESSIONS`, batido ao vivo durante o desenvolvimento). Cada processo Node faz login próprio, então o cron da ingestão compete com o app e com jobs manuais. ⚠️ O cron **foi agendado sem isto** (2026-08-04, ADR-0028) e passou a rodar **de hora em hora** — 24 execuções/dia contra as ~3 anteriores a zero. Mitigado pelo minuto `:20` (fora do `:00` dos outros crons) e pelo `BoundedConcurrency`, mas o usuário dedicado continua sendo a correção real. |
...
| 5 | ~~**Cron do `job:ingest-extratos`**~~ | ~~P2~~ → **RESOLVIDO (2026-08-04)** | Agendado via `.github/workflows/ingest-extratos.yml`: `20 * * * *` (horário), `concurrency` + `workflow_dispatch` + até 3 tentativas com backoff. O scheduler é o GitHub Actions e **não** in-process, então o job roda uma vez e não uma por instância web. Ver ADR-0028. Entregue **sem** o item 1 — que segue aberto e agravado. |
